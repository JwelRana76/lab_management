<x-admin title="Referral Report">
    <x-page-header head="Referral Report" />
    <x-card>
      <x-form action="{{ route('report.referral') }}" method="get">
        <div class="row m-auto">
          ...
          <div>
            ...
            <a href="{{ route('report.referral') }}" class="btn btn-sm btn-danger mb-3">Clear Finter</a>
            @endif
          </div>
        </div>
      </x-form>
    ...
    <div class="row">
      <x-data-table dataUrl="/report/referral" id="referralReport" :columns="$columns" />
    </div>
</x-admin>